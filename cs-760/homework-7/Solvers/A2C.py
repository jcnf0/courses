...
class A2C(AbstractSolver):
    ...
    def train(self, states, actions, rewards, next_state, done):
        ...
        next_states_tensor = torch.tensor([next_state], dtype=torch.float32)
        ...
        for k in range(len(returns)):
            returns[k] = sum(
                [self.options.gamma ** i * rewards[i] for i in range(k, len(rewards))]
            )

        returns[-1] += self.options.gamma ** len(rewards) * self.actor_critic.value(next_states_tensor).detach().numpy()
