...
class ValueIteration(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for each_state in range(self.env.nS):            
            # For each action compute the expected return
            expected_return = []
            for each_action in range(self.env.nA):
                # For each outcome of the action
                outcome = self.env.P[each_state][each_action]
                # For each possible outcome
                expected_return.append(sum([prob*(reward + self.options.gamma*self.V[next_state]) for prob, next_state, reward, done in outcome]))
            
            # Update the value of the state
            self.V[each_state] = max(expected_return)
